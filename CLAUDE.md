...
- **No AI attribution anywhere.** Never add a `Co-Authored-By: Claude …` /
  `Claude-Session: …` trailer to a commit, and never add a
  `🤖 Generated with Claude Code` line — or any other "generated by" / tool
  credit — to a commit message, a PR title or description, or a PR / code-review
  comment. Never push a commit that contains one. Commits are subject + body
  only; PRs and comments are plain prose.
