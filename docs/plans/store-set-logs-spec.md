...
- `App\Http\Controllers\Session\{LogSetController, UpdateSetLogController}` —
  invokable.
- `App\Actions\Session\{SetLogCreateAction, SetLogUpdateAction}`.
- `App\Services\Session\SetLoggingService` — the business invariants of writing a
  set: session `in_progress`, `day_exercise_id` belongs to *this* session's
  training day, `set_number` contiguous per exercise.
...
| Set-touching tests | None. | `tests/Feature/Exercise/ListExercisesTest.php`, `tests/Feature/Session/{LogSetTest, UpdateSetLogTest, SetLoggingServiceTest}.php`, `tests/Unit/Session/SetLogPolicyTest.php`, one `ArchTest` rule, three `DocsSecurityTest` assertions. |
...
### Action & Service — `tests/Feature/Session/SetLoggingServiceTest.php`
...
**TC-44:** `SetLoggingService::guardOpen()` throws for a completed session, passes for `in_progress`
- **Given:** a completed `TrainingSession` and an `in_progress` one
- **When:** `app(SetLoggingService::class)->guardOpen($session)` for each
- **Expect:** throws `App\Exceptions\Session\SessionAlreadyCompletedException` (`errorCode() === 'SESSION_ALREADY_COMPLETED'`, `statusCode() === 409`) for the completed one; returns `void` for the open one

**TC-45:** `SetLoggingService::resolveExercise()` rejects a `day_exercise` outside the session's training day
- **Given:** a `User`; `$session = openPlannedSession($user)`; a `DayExercise` from a **different** `CycleDay`; a `LogSetData` carrying its `uuid`
- **When:** `resolveExercise($session, $data)`
- **Expect:** throws `DayExerciseNotInSessionException` (`errorCode() === 'DAY_EXERCISE_NOT_IN_SESSION'`, `409`); a free session + any `day_exercise_id` throws the same

**TC-46:** `SetLoggingService::guardContiguousSetNumber()` — expected is `count + 1` per `(session, exercise)`
- **Given:** a session; an exercise with two logged sets (`set_number` 1, 2); a second exercise with none
- **When:** `guardContiguousSetNumber($session, $exerciseA, 3)`, `(…, $exerciseA, 2)`, `(…, $exerciseB, 1)`, `(…, $exerciseB, 2)`
- **Expect:** the `3` and `1` calls return `void`; the `2` (for A) and `2` (for B) calls throw `NonContiguousSetNumberException` (`errorCode() === 'NON_CONTIGUOUS_SET_NUMBER'`, `409`), message contains the expected number
...
- **Expect:** `App\Http\Controllers\Exercise` is invokable (new `arch(...)` line); the existing rules still pass — covering `SetLogCreateAction` / `SetLogUpdateAction` (`final` + `handle()`), `SetLoggingService` (`final`), `LogSetRequest` / `UpdateSetLogRequest` (extend `FormRequest`), and no debug helpers
...
| Session-completed guard | `SessionAlreadyCompletedException` → `409` `SESSION_ALREADY_COMPLETED`, from `SetLoggingService::guardOpen()`. Applies to `POST` and `PUT`. `SessionStatus` has only `in_progress` / `completed`, so "not completed" ≡ `in_progress`. | The revised AC5. The request is well-formed and the session is owned; the state forbids the write → `409`, not `403` / `422`. |
...
| Guard placement | `App\Services\Session\SetLoggingService` (`final`) holds three methods: `guardOpen(TrainingSession)`, `resolveExercise(TrainingSession, LogSetData): Exercise`, `guardContiguousSetNumber(TrainingSession, Exercise, int)`. The Actions orchestrate. | Three cross-entity business rules with three client-facing `code`s — real business knowledge to isolate (`CLAUDE.md` "Service" section). Mirrors `TrainingSessionOpeningService`. Keeps the Actions readable and gives the rules a unit-testable home. `SetLogUpdateAction` reuses `guardOpen` only. |
