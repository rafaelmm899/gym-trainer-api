...
| Guard placement | **No service.** The three guards live in the Actions: `SetLogCreateAction::handle()` reads top-to-bottom — completed check (`throw_if`), a private `resolveExercise()` (off-plan `exercise_id`, or a `day_exercise_id` that must belong to this session's day), the `count + 1` contiguity check, then the insert. `SetLogUpdateAction::handle()` is the completed check plus the update. | A `SetLoggingService` was drafted and rejected in review (`CLAUDE.md` rule 6): it wrapped one `throw_if`, one `firstOrFail` and one `count` for a single real caller and split the use-case story across two files. The completed check is one duplicated line across the two Actions — cheaper than a class. Behaviour stays covered white-box (`SetLogActionTest`) and over HTTP. |
...
| 9 | Create `app/Actions/Session/SetLogCreateAction.php` (`final`, **no constructor**): `handle(TrainingSession $session, LogSetData $data): SetLog` reads top-to-bottom — `throw_if($session->status === SessionStatus::Completed, new SessionAlreadyCompletedException)`; `$exercise = $this->resolveExercise($session, $data)`; `$nextNumber = $session->sets()->where('exercise_id', $exercise->id)->count() + 1; throw_unless($data->set_number === $nextNumber, new NonContiguousSetNumberException($nextNumber))`; `DB::transaction(fn () => $session->sets()->create(['exercise_id' => $exercise->id, 'set_number' => $data->set_number, 'weight_kg' => $data->weight_kg, 'reps' => $data->reps, 'rpe' => $data->rpe, 'note' => $data->note]))`; return `$set->load('exercise')`. Private `resolveExercise(TrainingSession $session, LogSetData $data): Exercise` — `exercise_id` → `Exercise::query()->where('uuid', $data->exercise_id)->firstOrFail()`; else `DayExercise::query()->with('exercise')->where('uuid', $data->day_exercise_id)->firstOrFail()` then `throw_unless($dayExercise->cycle_day_id === $session->cycle_day_id, new DayExerciseNotInSessionException)`, return `$dayExercise->exercise`. | `final` + `handle()`; covered by TC-42…TC-46; no strict-mode lazy-load; Pint + PHPStan clean. |
| 10 | Create `app/Actions/Session/SetLogUpdateAction.php` (`final`, **no constructor**): `handle(TrainingSession $session, SetLog $set, UpdateSetLogData $data): SetLog` — `throw_if($session->status === SessionStatus::Completed, new SessionAlreadyCompletedException)`; `DB::transaction(fn () => $set->update(['weight_kg' => …, 'reps' => …, 'rpe' => …, 'note' => …]))`; return `$set->load('exercise')`. Write `tests/Feature/Session/SetLogActionTest.php` (TC-42…TC-47). | Both Actions `final` + `handle()`; `vendor/bin/pest tests/Feature/Session/SetLogActionTest.php` green; Pint + PHPStan clean. |
