...
| Response shape | `TrainingSessionResource` gains `note` and `perceived_effort` as plain scalar fields (no nested resource, no cast beyond Eloquent's native string/int). | `CLAUDE.md` rule 3: always a real Resource. Both are flat attributes with no relation, so no `whenLoaded()` / nested resource is warranted. |
...
| 3 | Update `app/Models/TrainingSession.php`: add `note`, `perceived_effort` to `#[Fillable]`; add `@property string|null $note` / `@property int|null $perceived_effort` PHPDoc | Pint + PHPStan clean; `(new TrainingSession)->getFillable()` includes both. |
