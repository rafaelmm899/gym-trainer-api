...
**Kind:** Greenfield Feature — the Session domain (table, model, enums, policy,
factory, pipeline) is entirely new. It carries a bounded **brownfield** change
to the shipped Cycle domain: the first cycle is born `active` instead of
`draft`, which edits `CycleStatus`, `CycleDraftService`, `CycleFactory`, the
`Cycle` model docblock and three existing Cycle/Routine tests (§10 tasks 2 and
9).
...
TC-1 … TC-21 exercise this endpoint end to end over HTTP — the only documented
entry point. TC-29 goes through `POST /api/v1/routines` (an existing endpoint).
The rest — TC-22 … TC-28, TC-30, TC-31 — are focused white-box tests on internal
seams (`TrainingSessionCreateAction`, the opening Service via that Action,
`TrainingSessionPolicy`, `CycleDraftService`, `RoutineCreateAction`, and the
`CycleStatus` enum); every behaviour they assert is also observable through one
of the HTTP tests above, and they exist only to pin the seam in isolation.
...
| `day` — presence & type | Optional. `day` = the `CycleDay` **public `uuid`**. Absent, `null`, `""` or whitespace-only → free session. `StoreTrainingSessionRequest` validates `['nullable', 'uuid', 'exists:cycle_days,uuid']`; `prepareForValidation()` rewrites a `""` / whitespace-only `day` to `null` (`$this->merge(['day' => null])`), the same mechanism `StoreRoutineRequest` uses for `hint`. A `null` `day` and an omitted `day` are equivalent downstream (`nullable` rule, DTO default `?string $day = null`). | The acceptance criteria require both planned and free sessions. `exists` is shape only (the row is real); "the row belongs to *this* routine's active cycle" is a cross-entity rule (below). |
...
| Server-set session fields | The Action passes `status = in_progress` and `started_at = now()`. `analysis_state` is **not** passed by the Action — the model carries `protected $attributes = ['analysis_state' => AnalysisState::Pending->value]`, mirroring the DB `default 'pending'` (and `Routine::$attributes` for `days_per_cycle`) so the `201` body serialises `pending` without a reload. `completed_at` / `conversation_id` stay `null`. | AC "nace sin completar". The user never supplies these. `analysis_state` / `conversation_id` belong to the analysis story (Order 130); the columns ship now to keep `training_sessions` a faithful realisation of `data-model.md` in one migration, and are inert here. Under `preventAccessingMissingAttributes` a recently-created model returns `null`, not the DB default, for an unset column — hence `$attributes`, not "let the DB fill it". |
| Mass assignment | `TrainingSession` `#[Fillable(['user_id', 'cycle_day_id', 'status', 'analysis_state', 'started_at', 'completed_at'])]`. The Action creates through `$routine->trainingSessions()->create([...])` — the relation sets `routine_id`; `user_id` and `cycle_day_id` are passed explicitly (server-derived, never from the request body) and so must be fillable. | `Model::preventSilentlyDiscardingAttributes()` (strict mode, on outside prod) throws on a `create()` key that is not fillable. `user_id` / `cycle_day_id` in the fillable list mirrors `DayExercise` carrying `exercise_id`. They are never populated from `$request->validated()` (which only holds `day`), so there is no mass-assignment exposure. |
...
carries logic — its focused test authored in the same task. Task 19 (the
endpoint feature test) is the functional gate. Tasks 2 and 9 are the brownfield
slice (the `active`-cycle change); everything else is new.
...
| 8 | Create `database/factories/TrainingSessionFactory.php`: `user_id => User::factory()`, `routine_id => Routine::factory()`, **`cycle_day_id => null`** (default is a free session — the default `CycleDay::factory()` would build a `CycleDay` on an unrelated routine chain), `status => SessionStatus::InProgress`, `analysis_state => AnalysisState::Pending`, `started_at => now()`, `completed_at => null`; states `completed()` (`status => Completed`, `completed_at => now()`) and `planned(CycleDay $day)` (`cycle_day_id => $day->id`) | `TrainingSession::factory()->create()` (free), `->completed()->create()`, `->planned($day)->create()` each persist one row with a `uuid`; Pint + PHPStan clean. |
| 9 | **First cycle born `active`** (brownfield): in `database/factories/CycleFactory.php` set the default `status => CycleStatus::Active` + `activated_at => now()`, keep `active()`, add `draft()` (`status => Draft`, `activated_at => null`) and `incomplete()` (`status => Incomplete`, `activated_at => now()->subWeek()`, `completed_at => now()`); in `app/Services/Cycle/CycleDraftService.php::persist()` write `'status' => CycleStatus::Active` + `'activated_at' => now()`; update the `Cycle` model docblock ("born `active`"); update the assertions in `tests/Feature/Cycle/CycleDraftServiceTest.php` (TC-28), `tests/Feature/Routine/StoreRoutineTest.php` (TC-29) and `tests/Feature/Routine/RoutineCreateActionTest.php` (TC-30) from `'draft'` to `'active'` + `activated_at` non-null | `vendor/bin/pest tests/Feature/Cycle tests/Feature/Routine` green; `Cycle::factory()->create()->status === CycleStatus::Active`; `->draft()` / `->incomplete()` work; Pint + PHPStan clean. Lands as one change so the suites are never left inconsistent. |
| 10 | Create `app/Data/Session/CreateTrainingSessionData.php` (`make:data`, move to `app/Data/Session/`, fix namespace): `readonly ?string $day = null` | `CreateTrainingSessionData::from(['day' => $uuid])->day === $uuid`; `::from([])->day === null`; Pint + PHPStan clean. |
| 11 | Create `app/Exceptions/Session/SessionInProgressException.php`, `RoutineNotActiveException.php`, `CycleDayNotInActiveCycleException.php` — `final extends DomainException`; `$errorCode` = `SESSION_IN_PROGRESS` / `ROUTINE_NOT_ACTIVE` / `CYCLE_DAY_NOT_IN_ACTIVE_CYCLE`; each `__construct()` sets a fixed message | `(new SessionInProgressException)->errorCode()` / `->statusCode() === 409` for each; Pint + PHPStan clean. |
| 12 | Create `app/Policies/TrainingSessionPolicy.php`: `create(User $user, Routine $routine): bool => $routine->user_id === $user->id`. Write `tests/Unit/Session/TrainingSessionPolicyTest.php` (TC-27) | `vendor/bin/pest tests/Unit/Session/TrainingSessionPolicyTest.php` green; Pint + PHPStan clean. |
| 13 | Create `app/Services/Session/TrainingSessionOpeningService.php` (`final`): one method `guard(User $user, Routine $routine, ?CycleDay $day): void` — `throw_unless($routine->status === RoutineStatus::Active, new RoutineNotActiveException)`; if `$day !== null`, load `$cycle = $routine->cycle()->first()` and `throw_unless($cycle?->status === CycleStatus::Active && $day->cycle_id === $cycle->id, new CycleDayNotInActiveCycleException)`; `throw_if($user->trainingSessions()->where('status', SessionStatus::InProgress)->exists(), new SessionInProgressException)`. Order the checks routine → day → open-session. | `final`; Pint + PHPStan clean; no strict-mode lazy-load (uses `->cycle()->first()`); guard paths covered by the Action test (TC-24…TC-26). |
| 14 | Create `app/Http/Requests/Session/StoreTrainingSessionRequest.php`: `authorize()` → `$this->user()?->can('create', [TrainingSession::class, $this->route('routine')]) ?? false`; `rules()` → `['day' => ['nullable', 'uuid', 'exists:cycle_days,uuid']]`; `prepareForValidation()` rewrites a `""` / whitespace-only `day` to `null` (`$this->merge(['day' => null])`) | extends `FormRequest`; Pint + PHPStan clean; a whitespace `day` becomes `null`, a non-uuid `day` fails validation. |
| 15 | Create `app/Http/Resources/Session/TrainingSessionResource.php` (`@mixin TrainingSession`): `id => $this->uuid`, `status` / `analysis_state` via `->value`, `started_at` / `completed_at` / `created_at` / `updated_at` via `?->toIso8601String()`, `cycle_day => CycleDayResource::make($this->whenLoaded('cycleDay'))`; no internal ids, no `routine` | `toArray()` has no `user`/`routine`/`*_id` key; `id` is the uuid; Pint + PHPStan clean. |
| 16 | Create `app/Actions/Session/TrainingSessionCreateAction.php` (`final`, ctor injects `TrainingSessionOpeningService`): `handle(User $user, Routine $routine, CreateTrainingSessionData $data): TrainingSession` — resolve `$day = $data->day ? CycleDay::query()->where('uuid', $data->day)->first() : null`; `$this->guard->guard($user, $routine, $day)`; `DB::transaction(fn () => $routine->trainingSessions()->create([... 'status' => SessionStatus::InProgress, 'started_at' => now(), 'user_id' => $user->id, 'cycle_day_id' => $day?->id]))`; return `$session->load('cycleDay.dayExercises.exercise')` (a free session's `cycleDay` loads as `null`). Write `tests/Feature/Session/TrainingSessionCreateActionTest.php` (TC-22…TC-26) | `final` + `handle()`; `vendor/bin/pest tests/Feature/Session/TrainingSessionCreateActionTest.php` green; Pint + PHPStan clean. |
| 17 | Create `app/Http/Controllers/Session/StoreTrainingSessionController.php` (`make:controller --invokable`, move + fix namespace): `__invoke(StoreTrainingSessionRequest $request, Routine $routine, TrainingSessionCreateAction $action)` → build `CreateTrainingSessionData::from($request->validated())`, call the Action with `$request->user()`, `$routine`, the DTO, return `TrainingSessionResource::make($session)->response()->setStatusCode(Response::HTTP_CREATED)` | `final`, `__invoke` only; Pint + PHPStan clean. |
| 18 | Edit `routes/api.php`: add the `use` import and, in the `auth:sanctum` group, `Route::post('routines/{routine}/sessions', StoreTrainingSessionController::class)->whereUuid('routine')->name('routines.sessions.store')` | `php artisan route:list` shows `POST api/v1/routines/{routine}/sessions` with `auth:sanctum`; PHPStan clean in `routes/`. |
| 19 | Write `tests/Feature/Session/StoreTrainingSessionTest.php` covering TC-1 … TC-21 (`beforeEach` sets the `Origin` header + user + profile; the `activeRoutineWithCycle()` helper) | `vendor/bin/pest tests/Feature/Session/StoreTrainingSessionTest.php` all green; every TC-1 … TC-21 has a test. |
| 20 | Add `arch('session controllers are invokable')->expect('App\Http\Controllers\Session')->toBeInvokable()` to `tests/Feature/ArchTest.php` (TC-32); add the `/api/v1/routines/{routine}/sessions` `post` assertion to `tests/Feature/Auth/DocsSecurityTest.php` (TC-33) | `vendor/bin/pest tests/Feature/ArchTest.php tests/Feature/Auth/DocsSecurityTest.php` green. |
| 21 | `vendor/bin/pint --dirty`, then `vendor/bin/phpstan analyse`, then a final `php artisan ide-helper:models --write` + `vendor/bin/pint app/Models` | Pint reports no diffs; PHPStan level 6 clean; model PHPDoc in sync with the migration. |
| 22 | `composer check` (Pint `--test` + PHPStan level 6 + full Pest — new Session tests, the Policy test, the arch rule, the `DocsSecurityTest` addition, and the updated Cycle / Routine suites) | All three steps green; no regression in Auth / Profile / Routine / Cycle suites. |
| 23 | Manual check with `curl` against `http://localhost:8000` (worktree app pointed at the clone): register + login → `PUT /api/v1/profile` → `POST /api/v1/routines` (`201`, `cycle.status: active`) → `POST /api/v1/routines/{uuid}/sessions` with `{}` (`201`, free) → `POST` again (`409` `SESSION_IN_PROGRESS`) → with `{ "day": "<a day uuid>" }` after clearing the open row (`201`, embedded prescription) → foreign routine (`403`) → unknown routine uuid (`404`). Review `GET /docs/api` | The `curl` calls return the expected codes; the endpoint appears in Scramble with the request from `StoreTrainingSessionRequest` and the response from `TrainingSessionResource`, marked secured. |
| 24 | On merge / branch drop: `docker compose exec pgsql dropdb -U gym --if-exists gym_trainer_create_trainning_sessions`; revert `DB_DATABASE` in the worktree `.env` | The clone is gone; `.env` restored to `gym_trainer`. |
