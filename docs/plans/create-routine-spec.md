...
`HasPublicUuid` trait, the archival Service, the first concrete
`DomainException` subclass, and a placeholder `GenerateCycleJob`.
...
- `App\Services\Routine\RoutineActivationService` — the "single active slot"
  invariant: guard that the user has completed onboarding, and archive the
  incumbent active routine.
- `App\Exceptions\Profile\ProfileIncompleteException` — the concrete
  ...
  HTTP `409`, thrown from the Service guard when the user has no athlete
  profile.
- `App\Actions\Routine\RoutineCreateAction` — orchestrates the guard, the
  archival, the insert and the job dispatch in one transaction.
...
- The onboarding precondition ("the user must have an athlete profile") is **not**
  an authorization decision — it is a business rule, enforced by a Service guard
  that throws `ProfileIncompleteException` (`409`), not by the Policy (§9).
...
|---|---|---|
| Routine domain | No table, no model, no endpoint, no enum. `app/Services/`, `app/Jobs/`, `app/Policies/` do not exist. | `routines` table (many per user); `POST /api/v1/routines` creates one. `App\Enums\Routine\RoutineStatus`, `App\Services\Routine\RoutineActivationService`, `App\Jobs\Cycle\GenerateCycleJob`, `App\Policies\RoutinePolicy` are created. |
| Single active routine | Not represented. | Exactly one `active` routine per user, enforced by a partial unique index and by `RoutineActivationService::archiveCurrentActive()` archiving the incumbent inside the create transaction. |
...
| Domain exceptions | `App\Exceptions\DomainException` base + `ApiExceptionRenderer` exist (domain-exception PR); **no concrete subclass** ships (only test stubs). | `App\Exceptions\Profile\ProfileIncompleteException` — first real subclass (`PROFILE_INCOMPLETE` / `409`), thrown from `RoutineActivationService::guardCanCreate()`. |
...
| Onboarding precondition | Required. `RoutineActivationService::guardCanCreate()` throws `ProfileIncompleteException` (`409`) if `$user->athleteProfile()->doesntExist()`. | The routine exists to be fed to the AI cycle planner, which consumes the athlete profile (`docs/product-context.md` §4). Creating a routine with no profile would produce an ungeneratable cycle. Confirmed with the product owner. |
| Guard placement | In a **Service** (`RoutineActivationService`), not the Form Request, and not the Policy. | `CLAUDE.md`: business guard clauses and `DomainException` throws live in a Service; the Form Request is shape-only; a missing profile is account **state**, not an authorization decision. |
| Guard vs. archival in one Service | `RoutineActivationService` holds both `guardCanCreate()` and `archiveCurrentActive()`. | Both enforce one cohesive rule — "may this user take the single active-routine slot, and clear whoever holds it". Matches the `CLAUDE.md` `RoutineCreateAction` example, which injects exactly one Service. A separate Profile-domain guard is the alternative if the rule gets reused by cycle-generation / session tickets; deferred until then (rules 5–6). |
...
| Permanent archival mechanism | `archiveCurrentActive()` loads the user's `active` routine (if any) and sets `status = Archived` + `archived_at = now()` by explicit attribute assignment + `save()`, inside `RoutineCreateAction`'s transaction, before the insert. | AC #2. Explicit assignment keeps `archived_at` out of `$fillable` (only `RoutineActivationService` writes it). Running before the insert means the partial unique index is never violated. |
...
| Transaction boundary | `guardCanCreate()` runs **before** `DB::transaction`; the transaction wraps `archiveCurrentActive()` + insert + `dispatch()`. | The guard is a read-only precondition — fail fast, no transaction overhead. The mutations are atomic together. |
...
| 13 | Create `app/Services/Routine/RoutineActivationService.php` (`final`): `guardCanCreate(User $user): void` → `throw_if($user->athleteProfile()->doesntExist(), new ProfileIncompleteException())`; `archiveCurrentActive(User $user): void` → load the user's `active` routine, set `status`/`archived_at` and `save()`. No job dispatch, no events | File exists; Pint + PHPStan clean; unit-covered indirectly by TC-19 / TC-20. |
| 14 | Create `app/Jobs/Cycle/GenerateCycleJob.php` (`final implements ShouldQueue; use Queueable;`), ctor `public Routine $routine`, `handle(): void` empty with a docblock pointing to the "Recibir el primer ciclo" ticket | File exists; Pint + PHPStan clean; `GenerateCycleJob::dispatch($routine)` compiles. |
| 15 | Create `app/Actions/Routine/RoutineCreateAction.php` (`final`, ctor-promoted `RoutineActivationService`): `handle(User, RoutineData): Routine` — `guardCanCreate()`, then `DB::transaction`: `archiveCurrentActive()` → `$user->routines()->create([...'status' => RoutineStatus::Active])` → `GenerateCycleJob::dispatch($routine)` → return `$routine`. Write `tests/Feature/Routine/RoutineCreateActionTest.php` (TC-19, TC-20) | `final` + `handle()`; `vendor/bin/pest tests/Feature/Routine/RoutineCreateActionTest.php` green; Pint + PHPStan clean. |
| 16 | Create `app/Http/Controllers/Routine/StoreRoutineController.php` via `make:controller --invokable`, move + fix namespace: build `RoutineData::from($request->validated())`, call the Action, return `RoutineResource::make($routine)->response()->setStatusCode(Response::HTTP_CREATED)` | `final`, `__invoke` only; Pint + PHPStan clean. |
| 17 | Edit `routes/api.php`: add the `use` import and, in the existing `auth:sanctum` group, `Route::post('routines', StoreRoutineController::class)->name('routines.store')` | `php artisan route:list` shows `POST api/v1/routines` with `auth:sanctum`; PHPStan clean in `routes/`. |
| 18 | Write `tests/Feature/Routine/StoreRoutineTest.php` covering TC-1 … TC-18 (`beforeEach` sets the `Origin` header + `Bus::fake()`) | `vendor/bin/pest tests/Feature/Routine/StoreRoutineTest.php` all green; every TC-1 … TC-18 has a corresponding test. |
| 19 | Add `arch('routine controllers are invokable')->expect('App\Http\Controllers\Routine')->toBeInvokable()` to `tests/Feature/ArchTest.php` (TC-23); add the `/v1/routines` `post` assertion to `tests/Feature/Auth/DocsSecurityTest.php` (TC-24) | `vendor/bin/pest tests/Feature/ArchTest.php tests/Feature/Auth/DocsSecurityTest.php` green. |
| 20 | Run `vendor/bin/pint --dirty`, then `vendor/bin/phpstan analyse`, then a final `php artisan ide-helper:models --write` + `vendor/bin/pint app/Models` | Pint reports no diffs; PHPStan level 6 clean; the model PHPDoc is in sync with the migration. |
| 21 | Run `composer check` (Pint `--test` + PHPStan level 6 + full Pest — including the new `Routine` tests, the `RoutinePolicyTest`, the `DocsSecurityTest` addition and the arch rule) | All three steps green; no regression in Auth / Profile suites. |
| 22 | Manual check with `curl` against `http://localhost:8000`: `GET /sanctum/csrf-cookie` → register + login → `PUT /api/v1/profile` → `POST /api/v1/routines` (`201`, `status: active`) → `POST` again (`201`, previous routine now `archived`) → a second user with no profile → `POST` (`409`, `PROFILE_INCOMPLETE`) → invalid `goal` (`422` on the field) → no session (`401`). Review `GET /docs/api` | The `curl` calls return the expected codes; the endpoint appears in Scramble with the request inferred from `StoreRoutineRequest` and the response from `RoutineResource`, marked secured. |
| 23 | On merge / branch drop: `docker compose exec pgsql dropdb -U gym --if-exists gym_trainer_create_routings`; revert `DB_DATABASE` in the worktree `.env` | The clone is gone; `.env` is restored to `gym_trainer`. |
