...
- `App\Ai\Agents\Cycle\CyclePlannerAgent` — a `laravel/ai` agent implementing
  ...
  `config('ai.default')` (`anthropic`) and that provider's configured text model
  (`config('ai.providers.<driver>.models.text.default')`, from the single
  `AI_PROVIDER_MODEL` env var, defaulting per driver to
  `claude-haiku-4-5-20251001` / `gpt-5.4`) and a 60 s timeout.
...
- **Synchronous generation.** `RoutineCreateAction` calls
  ...
  **before** opening any transaction. The agent runs in-request (60 s timeout,
  provider text model from config). On success, one transaction archives the incumbent active
...
| `CYCLE_EXERCISES_PER_DAY_BEGINNER` / `_INTERMEDIATE` / `_ADVANCED` | `.env` / `.env.example`, defaults `3-5` / `4-6` / `5-8` (a `"min-max"` string) | Exercises the planner prescribes per training day, by athlete experience level. Backs `config('training.cycle.exercises_per_day.*')`. |

`.env.example` gains the three `CYCLE_EXERCISES_PER_DAY_*` keys. `phpunit.xml` already sets `DB_CONNECTION=sqlite`,
...
| Shape validation location | In `CyclePlannerService`, after the agent returns: exactly 5 days; each day within the athlete's **experience-level exercise range** (`config('training.cycle.exercises_per_day.<level>')`, a `"min-max"` string, per-level env-overridable — default beginner `3-5`, intermediate `4-6`, advanced `5-8`); `rep_min ≤ rep_max`; `sets ≥ 1`; `rest_seconds ≥ 0`; `target_weight_kg` present and `≥ 0`; every `focus_muscle_groups` entry a `MuscleGroup`. Any violation → `CycleGenerationException`. The same range is stated in the prompt ("Prescribe between {min} and {max} exercises on EVERY day"). | The structured-output JSON schema constrains a well-behaved provider; the explicit check defends against a bad provider **and** malformed test fakes, and is the single place the "unusable plan → 502" rule lives. The exercise count is athlete-specific so it lives in the service + prompt, not the static schema. |
...
| AI model | `CyclePlannerAgent` carries **no model attribute** — it uses `config('ai.default')` (`anthropic`) and that provider's configured text model. `config/ai.php` gains `providers.anthropic.models.text.default` and `providers.openai.models.text.default`, both `env('AI_PROVIDER_MODEL', <per-driver default>)`. `#[Timeout(60)]` stays. *(Follow-up to the merged story: was `#[UseCheapestModel]`; changed so the model is one `.env` knob — `AI_PROVIDER_MODEL`, alongside `AI_PROVIDER` — rather than a code attribute or a cheapest/smartest split.)* | Confirmed with the product owner. A single env var is the whole surface; unset it and each driver keeps a sane default (haiku / gpt-5.4). The call is in the request path, so **60 s** (not 120 s) bounds the worst-case HTTP latency. |
...
| `CyclePlannerAgent` | `final implements Agent, HasStructuredOutput { use Promptable; }`; `#[Timeout(60)]` (no model attribute — provider text model from config); `instructions(): string` (trainer role, "exactly 5 training days", kilograms, the `MuscleGroup` vocabulary, "always set a starting target weight from experience level", required per-day / per-exercise fields); `schema(JsonSchema $schema): array` — **strict-mode compatible**: every object marks all its properties in `required` and sets `additionalProperties: false` (a logically-optional field like `primary_muscle_group` / `target_rpe` stays required but nullable); `minItems`/`maxItems` on `days` (fixed at 5), **none on `exercises`** (that count is athlete-specific — prompt + service), `->min(1)` on `focus_muscle_groups`, `->enum(MuscleGroup::values())`. No constructor. | `data-model.md` §5. OpenAI / Groq `json_schema` mode rejects a schema whose objects omit keys from `required`, and enforces the bounds server-side. `laravel/ai` returns a `StructuredAgentResponse` (array-accessible) when the agent implements `HasStructuredOutput`; `Promptable` gives `::make()` / `->prompt()` / `::fake()` / `::assertPrompted()`. |
...
| 10 | Create `app/Ai/Agents/Cycle/CyclePlannerAgent.php` per §9 (`#[Timeout(60)]`, no model attribute, `instructions()`, `schema()` with bounds + `MuscleGroup` enum); add `models.text.default` (from `AI_PROVIDER_MODEL`) to the `anthropic` / `openai` blocks of `config/ai.php` and document `AI_PROVIDER_MODEL` in `.env.example`. | Pint + PHPStan clean; `CyclePlannerAgent::fake([...]); CyclePlannerAgent::make()->prompt('x')->toArray()` returns the fake payload; `config('ai.providers.anthropic.models.text.default')` follows `AI_PROVIDER_MODEL`. |
