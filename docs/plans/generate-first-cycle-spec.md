...
- `App\Ai\Agents\Cycle\CyclePlannerAgent` — a `laravel/ai` agent implementing
  ...
  (`config('ai.providers.anthropic.models.text.default')`, from the
  `ANTHROPIC_MODEL` env var, defaulting to `claude-haiku-4-5-20251001`) and a
  60 s timeout.
...
| AI model | `CyclePlannerAgent` carries **no model attribute** — it uses `config('ai.default')` (`anthropic`) and that provider's configured text model. `config/ai.php` gains `providers.anthropic.models.text.default = env('ANTHROPIC_MODEL', 'claude-haiku-4-5-20251001')` (and the same for `openai` / `OPENAI_MODEL`). `#[Timeout(60)]` stays. *(Follow-up to the merged story: was `#[UseCheapestModel]`; changed so the model is a single per-driver `.env` knob rather than a code attribute or a cheapest/smartest split.)* | Confirmed with the product owner. One env var per provider is the whole surface; the default keeps the merged behaviour (haiku). The call is in the request path, so **60 s** (not 120 s) bounds the worst-case HTTP latency. |
...
| 10 | Create `app/Ai/Agents/Cycle/CyclePlannerAgent.php` per §9 (`#[Timeout(60)]`, no model attribute, `instructions()`, `schema()` with bounds + `MuscleGroup` enum); add `models.text.default` (from `ANTHROPIC_MODEL` / `OPENAI_MODEL`) to the `anthropic` / `openai` blocks of `config/ai.php` and the two vars to `.env.example`. | Pint + PHPStan clean; `CyclePlannerAgent::fake([...]); CyclePlannerAgent::make()->prompt('x')->toArray()` returns the fake payload; `config('ai.providers.anthropic.models.text.default')` follows `ANTHROPIC_MODEL`. |
