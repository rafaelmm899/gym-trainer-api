...
- **When:** the session is completed
- **Expect:** `assertDatabaseCount('exercise_recommendations', 3)`, one row per exercise; `SessionAnalystAgent::assertPromptedTimes(1)` (a single call analyzed all three)
...
- **Expect:** `200`; `data.status === 'completed'`; `assertDatabaseHas('training_sessions', ['id' => $session->id, 'status' => 'completed'])`; reloading the session, `analysis_state === 'failed'` (the fake throws on every call, so all 3 `tries` are exhausted within the single synchronous dispatch — see §9); `assertDatabaseCount('exercise_recommendations', 0)`
...
- **Expect:** `SessionAnalystAgent::assertNotPrompted()` — the job never ran because the transaction it was deferred to never committed; no `training_sessions` or `exercise_recommendations` row persists
...
| 11 | Add `sessionAnalysisPayload()` / `fakeSessionAnalyst()` to `tests/Helpers.php` per §8; write `tests/Feature/Session/SessionAnalysisJobTest.php` (TC-1…TC-10) | `vendor/bin/pest tests/Feature/Session/SessionAnalysisJobTest.php tests/Unit/Recommendation/SessionAnalystServiceTest.php` all green. |
