...
- **When:** `POST /api/v1/routines/{routine}/cycles`
- **Expect:** `201`; `CyclePlannerAgent::assertPrompted(...)` — the prompt names that exercise with `performed = false` and the "keep the same target" instruction (exact string asserted against the built prompt, per `buildNextCyclePrompt()`)
