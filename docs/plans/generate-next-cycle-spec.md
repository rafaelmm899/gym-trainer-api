...
| Real performance, never the standing recommendation, drives `actual*` / trend | `actualAvgWeightKg` / `actualAvgReps` / `actualMaxRpe` and the trend/plateau signal are computed **exclusively from `SetLog` rows** — what the athlete actually lifted — never from `ExerciseRecommendation.target_weight_kg` (the *previous* suggested target). Example: a day prescribes 82.5kg, the standing recommendation says 85kg, the athlete actually logs 100kg — `actualAvgWeightKg = 100`, not `85`. Concretely, this means the planner sees two independent, mutually-reinforcing signals for that exercise: (1) the recommendation, which by generation time is already the **result** of the athlete's most recent session-close analysis (`SessionAnalyzeAction`, shipped, unaffected by this ticket) — that analysis itself is fed the literal sets logged that session, not the prior recommendation, so a 100kg performance against an 85kg target already produces an advanced recommendation (e.g. `110kg`) the moment that session closes, well before any cycle rollover; and (2) this ticket's own recomputation of the same real numbers straight from `SetLog`, independent of whatever the recommendation row says. Neither signal ever treats a stale recommendation value as authoritative over what was actually performed. | Raised and confirmed during spec review: "las recomendaciones dependen de lo que hizo el usuario y NO de la recomendación anterior... si el usuario hace 100kg, realmente lo que vale fue esos 100kg". Recorded here so a future reader doesn't need to re-derive it from the two services' code. |
