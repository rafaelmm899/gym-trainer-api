...
- **When:** the OpenAPI document is generated (`app(\Dedoc\Scramble\Generator::class)()` — same output the `GET /docs/api.json` route serves; the route itself is gated by `RestrictedDocsAccess` outside `local`)
- **Expect:** exactly one `components.securitySchemes` entry, `type: apiKey` with `in: cookie` (not `http`/`bearer`); a non-empty root `security` applying that scheme globally; the `auth:sanctum` operations (`paths./v1/logout.post`, `paths./v1/user.get` — Scramble strips the `api` prefix and serves it as the base path) carry **no** operation-level `security` override, so they inherit the global requirement; the public operations (`paths./v1/login.post`, `paths./v1/register.post`) set `security: []`
...
| 12 | Enable `config/scramble.php` `security_strategy` = `[MiddlewareAuthSecurityStrategy::class, ['middleware' => ['auth:sanctum'], 'scheme' => SecurityScheme::apiKey('cookie', env('SESSION_COOKIE', '<app>-session'))]]` (`SecurityScheme` factory confirmed against `dedoc/scramble` v4); write `tests/Feature/Auth/DocsSecurityTest.php` (TC-21) | PHPStan clean in `config/`; `pest tests/Feature/Auth/DocsSecurityTest.php` green — one `securitySchemes` entry (`type: apiKey`, `in: cookie`), non-empty root `security`, `auth:sanctum` operations inherit it (no override), `/v1/login` and `/v1/register` set `security: []`. |
